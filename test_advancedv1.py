...
class HumanMouseMover:
    def __init__(self):
        ...
        self.human_params = {
            ...
            'jitter_amount': 0,     # Убрана тряска
            'pause_variation': 0.2, # Меньше вариация пауз
            'curve_intensity': 1.0, # Увеличенная кривизна для синусоид
        }
    ...
    def human_sinusoid_move(self, start_x, start_y, end_x, end_y, duration_factor=1.0):
        ...
        frequency = random.uniform(1.2, 2.0)  # Средняя частота
        amplitude = random.uniform(25, 60) * (distance / 500)  # Увеличенная амплитуда
        
        angle = math.atan2(end_y - start_y, end_x - start_x)
        steps = max(10, int(duration * 30))  # Увеличенное количество шагов для плавности
    ...
    def smooth_move_with_action(self, start_x, start_y, end_x, end_y, action="move"):
        ...
        # 50/50 прямые и синусоидальные движения
        move_types = [
            (self.fast_linear_move, 0.50),     # 50% - быстрые прямые
            (self.human_sinusoid_move, 0.50),  # 50% - синусоидальные движения
        ]
...
def main():
    ...
    print("\nОСНОВНЫЕ УЛУЧШЕНИЯ:")
    print("  • 50/50 прямые и синусоидальные движения")
    ...
    print("  • Переключение вкладок каждые 4 минуты")
...
if __name__ == "__main__":
    ...
    print("Убедитесь, что нет открытых важных окон!")
